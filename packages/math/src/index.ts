...
export function adjustBusinessDay(
  ...
  if (convention === 'Unadjusted') return new Date(date);
  ...
  const isNonBusiness = (d: Date): boolean => {
    const day = d.getUTCDay();
  ...
  if (!isNonBusiness(current)) return current;
...
export function amortize(input: AmortizeInput): AmortizeResult {
  ...
  if (activeMonths > 0) {
    if (annualRate <= 0) {
      ...
      basePayment = round2(
        ((balance - balloonPayment / factor) * monthlyRate * factor) / (factor - 1)
      );
    }
 ...
 */
export function xirr(cashflows: CashflowPoint[], guess = 0.1): number {
  ...
  const calcNPV = (rate: number): number => {
    ...
    for (const flow of normalized) {
      ...
      if (rate <= -1.0) return Number.NEGATIVE_INFINITY;
      npv += flow.amount / Math.pow(1 + rate, years);
    }
    return npv;
  };

  const calcDNPV = (rate: number): number => {
    let dnpv = 0;
    for (const flow of normalized) {
      const years = (flow.date.getTime() - originTime) / (365 * 24 * 60 * 60 * 1000);
