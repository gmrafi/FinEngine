...
export function initQuickNav() {
  ...
  function buildEntries() {
    ...
    QUICK_ENTRIES.push(
      ...
      { label: '@finengine/core', group: 'Packages', href: `${root}docs/core/` },
      { label: '@finengine/math', group: 'Packages', href: `${root}docs/math/` },
      { label: '@finengine/ui', group: 'Packages', href: `${root}docs/ui/` },
      { label: 'Examples gallery', group: 'Packages', href: `${root}docs/examples/` },
      { label: 'Student Installment Simulator', group: 'Simulators', href: `${root}simulation/#simulator-student` },
      { label: 'SME Working Capital Simulator', group: 'Simulators', href: `${root}simulation/#simulator-sme` },
      { label: 'Merchant Restock Simulator', group: 'Simulators', href: `${root}simulation/#simulator-merchant` },
      { label: 'Release v0.3.0', group: 'Resources', href: 'https://github.com/gmrafi/FinEngine/releases/tag/v0.3.0' },
      { label: 'Report an issue', group: 'Resources', href: 'https://github.com/gmrafi/FinEngine/issues/new/choose' },
      { label: 'GitHub repository', group: 'Resources', href: 'https://github.com/gmrafi/FinEngine' },
    );
  ...
  function render(filter = '') {
    ...
    const entries = buildEntries().filter((e) => !q || `${e.label} ${e.group}`.toLowerCase().includes(q));
    if (!entries.length) {
  ...
  overlay.querySelector('[data-quicknav-close]').addEventListener('click', close);
